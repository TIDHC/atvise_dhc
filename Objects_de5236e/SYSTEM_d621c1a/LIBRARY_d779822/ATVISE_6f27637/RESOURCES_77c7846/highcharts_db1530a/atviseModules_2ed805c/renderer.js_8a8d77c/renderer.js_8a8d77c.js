...
Renderer.prototype.runContinuousRendering = function () {
	...
	function updateExtremes() {
		if (typeof self.chart.xAxis != "undefined" && self.chart.xAxis.length > 0) {
			...
			var zoomWheel = self.chart._zoom && self.chart._zoom.wheelZoomed ? self.chart._zoom.wheelZoomed : false;
			...
			if (self.zoomReset) {
				...
				self.zoomMaxOffset = until - self.zoomExtremes[0].max;
				self.zoomMinOffset = self.zoomExtremes[0].min - from;
				self.zoomExtremes = null;

				self.chart.xAxis[0].setExtremes(from + self.zoomMinOffset, until - self.zoomMaxOffset, false, false);
			} else if (zoomWheel) {
				let wheelMinX = from + self.chart._zoom.wheelExtremesDeltaX * self.chart._zoom.wheelFactor;
				let wheelMaxX = until - self.chart._zoom.wheelExtremesDeltaX * self.chart._zoom.wheelFactor;

				self.chart.xAxis[0].userMin = wheelMinX;
				self.chart.xAxis[0].userMax = wheelMaxX;
				self.chart.xAxis[0].options.userMin = wheelMinX;
				self.chart.xAxis[0].options.userMax = wheelMaxX;

				self.chart.xAxis[0].setExtremes(wheelMinX, wheelMaxX, false, false);
			} else {
				self.chart.xAxis[0].setExtremes(from + self.zoomMinOffset, until - self.zoomMaxOffset, false, false);
			}
		...
		for (var dataKey in data) {
			...
			if (series) {
				try {
					...
					if (dataKey.indexOf("--high-value") !== -1) {
