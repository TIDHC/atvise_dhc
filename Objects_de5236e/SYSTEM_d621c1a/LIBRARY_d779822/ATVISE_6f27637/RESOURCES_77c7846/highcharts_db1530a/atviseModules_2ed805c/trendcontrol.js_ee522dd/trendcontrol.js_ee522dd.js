...
TrendControl.decorate = function (control, chart) {
	...
	control.updated = function (exec, callback) {
		...
		/** Normalize empty source options in order to fix subsequent comparisons.
		 * [AT-D-15158] */
		if (oldConfig.options.atviseOptions.sourceOptions === "") oldConfig.options.atviseOptions.sourceOptions = "{}";
		if (chart.options.atviseOptions.sourceOptions === "") chart.options.atviseOptions.sourceOptions = "{}";
	...
	function onHistoryLoadedCallback() {
		if (chart === null) return; // Chart has been destroyed in the interim
