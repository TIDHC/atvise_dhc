...
(function (H) {
	...
	/**
	 * Fix removing point after series resets
	 * (e.g. change node of series) AT-D-15679
	 */
	H.wrap(H.Point.prototype, 'remove', function (proceed, optionsObj) {
		if(this.series)
			proceed.apply(this, [optionsObj]);
	});
	...
	H.Series.prototype.addLivePoint = function (pointArray, arg2, arg3, highPoint) {
		...
		/* --------------------------------------------------------------------- */
		/* fire add live point event to notify legend while history is loading   */
		var setAddLivePointEvent = new CustomEvent('trendAddLivePoint', {
			detail: {
				series: this.options.id,
				pointArray: pointArray
			}
		});

		this.chart.renderTo.dispatchEvent(setAddLivePointEvent);

		/* --------------------------------------------------------------------- */
		/* Add nonstop values and continue                                       */
